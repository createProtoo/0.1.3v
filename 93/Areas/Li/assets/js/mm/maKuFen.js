...
$(document).ready(function () {
    ...
    $(document).off("click", ".huabei").on("click", ".huabei", function () {
        ...
        hub.server.setRunQrcodeIsHuaBei(tmpId,false).done(function (result) {
            if (result == 1) {
                alert('设置成功!');
                loadRunsData();
            } else {
    ...
    tableListData4 = $('#maKu-datatables').DataTable({
        ...
        "columnDefs": [//列定义
            ...
            },
            {
                ...
                "render": function (data, type, row, meta) {
                    ...
                    //   // alert(tmpQrcodeUrl);
                    //}
                    var HTML = "<button style='height:50%;display:none' data-id=" + tableListData4.myID + " class='btn btn-icon btn-link budan' type='button'><i class='far fa-edit'></i></button>" +
                        "<button style='height:50%;display:none' data-id=" + tableListData4.myID + " class='btn btn-icon btn-link budan' type='button'><i class='far flaticon-cross'></i></button>" +
        ...
        "rowCallback": function (row, data, index) {
            ...
            if (data.qrTime != null) {
                ...
                $('td:eq(11)', row).html('<span>' + GetOKTime(data.qrTime)+ '</span>');
            }
